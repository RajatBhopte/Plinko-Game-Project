...
import { playTick, playWin, playLoss } from '../utils/audio';
...
export const PlinkoBoard: React.FC<PlinkoBoardProps> = ({ 
  ...
  const { finishDrop, multipliers } = usePlinkoGame();
  ...
  useEffect(() => {
    ...
    // Layout Calculation
    // We adjust padding based on available height to ensure things fit
    // On very small screens, reduce top padding to maximize grid space
    const paddingTop = Math.min(40, height * 0.05); 
    // Reduced minimum bottom padding to ensure it fits in 300px height
    ...
    const maxCols = 15; 
    const centerIndex = 7;
    // We constrain column width by both width AND height to keep aspects reasonable
    // Ensure pegs don't get too vertically stretched or squashed
    ...
    const render = () => {
      ...
      multipliers.forEach((mult, i) => {
        ...
        const activeFontSize = isLanded ? Math.min(fontSize * 1.3, colWidth * 0.7) : fontSize;

        ctx.fillStyle = isLanded ? '#fff' : (mult >= 1 ? '#4ade80' : '#f87171');
        ctx.font = `bold ${activeFontSize}px "Orbitron", sans-serif`;
      ...
      for (let r = 0; r < ROWS; r++) {
        ...
        for (let c = minCol; c <= maxCol; c++) {
           const cx = (c + (r % 2 === 0 ? 0 : 0.5)) * colWidth + (width - (15 * colWidth)) / 2;
      ...
      if (animState.current.active && path) {
        ...
        if (s.progressBetweenRows >= 1) {
            ...
            if (s.currentRow < path.length) {
                ...
                // Landed
                s.active = false;
                const final = Math.round(s.targetCol); // Should map to 0-12
                setLastLandedBin(final);
                finishDrop();
                
                const finalMult = multipliers[Math.min(multipliers.length-1, Math.max(0, final))] || 0;
                
                if (finalMult >= 1) {
                    playWin(finalMult);
                    const landX = (final + 1) * colWidth + (width - (15 * colWidth)) / 2;
        ...
        const currentXGrid = (s.startCol + (s.targetCol - s.startCol) * s.progressBetweenRows) + 1;
        const currentYGrid = (s.currentRow === -1 ? -0.5 : s.currentRow) + s.progressBetweenRows;
  ...
  }, [dimensions, dropColumn, path, multipliers, finishDrop, lastLandedBin]);

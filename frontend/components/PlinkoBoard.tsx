...
export const PlinkoBoard: React.FC<PlinkoBoardProps> = ({
  ...
  // Animation state - FIXED variable names
  const animState = useRef({
    active: false,
    currentRow: -1,
    progressBetweenRows: 0,
    currentBinPos: 0,     // Current bin position (0-12)
    targetBinPos: 0,      // Target bin position
    startBinPos: 0,       // Start of current segment
  });
  ...
  // Initialize drop
  useEffect(() => {
    if (isDropping && path && path.length > 0 && !animState.current.active) {
      console.log("🎮 ANIMATION INIT:");
      console.log("  dropColumn:", dropColumn);
      console.log("  path:", path);
      console.log("  finalBin from backend:", finalBin);
      ...
      animState.current = {
        ...
        currentRow: -1,
        progressBetweenRows: 0,
        currentBinPos: dropColumn,  // Start at chosen column
        targetBinPos: dropColumn,
        startBinPos: dropColumn,
      };
    }
  }, [isDropping, path, dropColumn, finalBin]);
  ...
  useEffect(() => {
    ...
    const binWidth = Math.min(width / BINS, rowHeight * 1.2);
    const boardWidth = binWidth * BINS;
    const boardLeft = (width - boardWidth) / 2;

    const pegRadius = binWidth * 0.12;
    const ballRadius = pegRadius * 1.8;
    ...
    const render = () => {
      ...
      // Draw Bins
      const binY = paddingTop + ROWS * rowHeight + 20;
      const showSuffix = binWidth > 32;

      multipliers.forEach((mult, i) => {
        const x = boardLeft + (i + 0.5) * binWidth;
        const isLanded = lastLandedBin === i;

        ctx.fillStyle = isLanded ? "rgba(34, 211, 238, 0.2)" : "rgba(15, 23, 42, 0.6)";
        ctx.beginPath();
        const taper = 2;
        ctx.moveTo(x - binWidth / 2 + taper, binY);
        ctx.lineTo(x + binWidth / 2 - taper, binY);
        ...
        let fontSize = Math.floor(binWidth * 0.45);
        if (fontSize < 8) fontSize = 8;
        if (fontSize > 13) fontSize = 13;
        const activeFontSize = isLanded ? Math.min(fontSize * 1.3, binWidth * 0.7) : fontSize;
      ...
      for (let r = 0; r < ROWS; r++) {
        const pegsInRow = r + 1;
        const startBin = (BINS - 1 - r) / 2;
        for (let p = 0; p <= r; p++) {
          const binPos = startBin + p;
          const cx = boardLeft + (binPos + 0.5) * binWidth;
          const cy = paddingTop + r * rowHeight;
          ctx.beginPath();
          ctx.arc(cx, cy, pegRadius, 0, Math.PI * 2);
          ctx.fill();
      ...
      // Ghost Ball
      if (!animState.current.active) {
        const startX = boardLeft + (dropColumn + 0.5) * binWidth;
        ctx.beginPath();
        ctx.arc(startX, paddingTop - 20, ballRadius, 0, Math.PI * 2);
        ctx.fillStyle = "rgba(34, 211, 238, 0.3)";
        ctx.fill();
      ...
      // Active Ball Animation - FIXED
      if (animState.current.active && path) {
        const s = animState.current;
        const speed = prefersReducedMotion.current ? 0.15 : 0.08;
        s.progressBetweenRows += speed;

        if (s.progressBetweenRows >= 1) {
          s.currentRow++;
          s.progressBetweenRows = 0;
          s.startBinPos = s.targetBinPos;

          if (s.currentRow < ROWS && s.currentRow >= 0) {
            playTick();
          }

          // FIXED: Correct movement logic
          if (s.currentRow < path.length && s.currentRow < ROWS) {
            const move = String(path[s.currentRow]).toUpperCase();
            
            if (move === "R" || move === "RIGHT") {
              s.targetBinPos = s.startBinPos + 1; // Move RIGHT = +1
            } else {
              s.targetBinPos = s.startBinPos;      // Move LEFT = stay
            }
          } else {
            // Landed - USE BACKEND'S finalBin
            s.active = false;
            const landedBin = finalBin !== undefined ? finalBin : s.targetBinPos;
            
            console.log("🏁 Landed! Backend finalBin:", finalBin, "Animation pos:", s.targetBinPos);
            
            setLastLandedBin(landedBin);
            onFinishDrop();

            const finalMult = multipliers[landedBin] || 0;
            if (finalMult >= 1) {
              playWin(finalMult);
              const landX = boardLeft + (landedBin + 0.5) * binWidth;
              spawnConfetti(landX, binY);
            } else {
              playLoss();
            }
        ...
        const currentBinPos = s.startBinPos + (s.targetBinPos - s.startBinPos) * s.progressBetweenRows;
        const currentYRow = (s.currentRow === -1 ? -0.5 : s.currentRow) + s.progressBetweenRows;

        const pixelX = boardLeft + (currentBinPos + 0.5) * binWidth;
        const pixelY = paddingTop + currentYRow * rowHeight;
    ...
    render();
    return () => cancelAnimationFrame(requestID);
  }, [dimensions, dropColumn, path, finalBin, multipliers, onFinishDrop, lastLandedBin, isMuted]);

  return (
    <div ref={containerRef} className="w-full h-full relative rounded-xl overflow-hidden cursor-crosshair">
      <canvas ref={canvasRef} className="block w-full h-full" />
    </div>
  );
};

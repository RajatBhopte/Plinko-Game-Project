      ...
      <div className="bg-slate-800 p-8 rounded-2xl border border-slate-700 shadow-xl">
        ...
        <form onSubmit={handleVerify} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-slate-400">Server Seed</label>
                <input 
                    type="text" 
                    value={serverSeed}
            ...
            <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-slate-400">Client Seed</label>
                <input 
                    type="text" 
                    value={clientSeed}
            ...
            <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-slate-400">Nonce</label>
                <input 
                    type="number" 
                    value={nonce}
            ...
            <div className="space-y-2">
                <label className="text-xs font-bold uppercase text-slate-400">Drop Column</label>
                <input 
                    type="number" 
                    min="0" max="12"
                    value={dropColumn}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-fade-in">
              ...
              <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 flex flex-col gap-4">
                  <div className="flex items-center gap-3 mb-2">
                      ...
                      <h3 className="text-xl font-bold text-green-400">Verified Outcome</h3>
                  </div>
                  
                  <div className="space-y-1">
                      <label className="text-[10px] uppercase text-slate-500 font-bold">Combined Seed Hash (SHA256)</label>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                      <div className="bg-slate-900 p-3 rounded border border-slate-700">
                          <label className="text-[10px] uppercase text-slate-500 font-bold">Final Bin</label>
                          <div className="text-2xl font-mono text-cyan-400">{result.finalBin}</div>
                      </div>
                      <div className="bg-slate-900 p-3 rounded border border-slate-700">
                          <label className="text-[10px] uppercase text-slate-500 font-bold">Multiplier</label>
                          <div className="text-2xl font-mono text-green-400">{result.payout}x</div>
                      </div>
                  </div>

                  <div className="mt-2">
                      <label className="text-[10px] uppercase text-slate-500 font-bold">Path</label>
                      <div className="flex gap-1 flex-wrap mt-1">
                          {result.path.map((dir, i) => (
                              <span key={i} className={`text-xs font-bold px-1.5 py-0.5 rounded ${dir === 'L' ? 'bg-indigo-900 text-indigo-200' : 'bg-purple-900 text-purple-200'}`}>
                                  {dir}
                              </span>
                          ))}
                      </div>

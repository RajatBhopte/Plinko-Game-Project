...
const PayTableLegend = () => {
    ...
    return (
        <div className="mt-2">
            <label className="text-slate-500 text-[10px] font-bold uppercase tracking-wider mb-1 block">Paytable (xMultipliers)</label>
                ...
                {payouts.map((p, i) => {
                    ...
                    return (
                        <div key={i} className="flex-1 flex flex-col justify-end items-center group relative">
                            <div className={`w-full rounded-t-sm ${color} transition-all hover:bg-cyan-400`} style={{ height }}></div>
                            <span className="text-[9px] text-slate-400 mt-1 opacity-0 group-hover:opacity-100 absolute -bottom-4 origin-center scale-75 sm:scale-100 whitespace-nowrap z-10 bg-slate-900/90 px-1 rounded border border-slate-700">{p}x</span>
                        </div>
                    )
                })}
            </div>
  ...
  return (
    <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 shadow-xl flex flex-col gap-4">
      ...
      <div className="space-y-1">
        <label className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Bet Amount ($)</label>
        <div className="relative group">
          ...
          <div className="absolute right-1 top-1/2 -translate-y-1/2 flex gap-1 bg-slate-800/50 p-1 rounded-md backdrop-blur-sm">
            <button 
                onClick={() => setBetAmount(Math.max(1, parseFloat((betAmount / 2).toFixed(2))))}
      ...
      <div className="space-y-2">
        <div className="flex justify-between items-center">
            <label className="text-slate-400 text-[10px] font-bold uppercase tracking-wider">Drop Position</label>
            <span className="font-mono text-cyan-400 font-bold text-sm">{dropColumn}</span>
        </div>
        <input
          type="range"
          min="0"
      ...
      {showFairness && setClientSeed && (
          <div className="bg-slate-900 rounded-lg border border-slate-700 overflow-hidden animate-fade-in">
              <div className="bg-slate-950/50 px-3 py-1.5 border-b border-slate-700/50 flex justify-between items-center">
                  <span className="text-[9px] font-bold uppercase text-slate-500 tracking-wider">Provably Fair</span>
                  <div className="flex items-center gap-2">
                     <span className="text-[9px] text-slate-500 uppercase font-bold">Nonce</span>
                     <span className="text-[10px] font-mono text-cyan-400 bg-cyan-900/20 px-1 rounded">{nonce}</span>
                  </div>
              </div>
              
              <div className="p-3 space-y-2">
                  <div>
                      <label className="text-slate-400 text-[10px] block mb-1">Client Seed</label>

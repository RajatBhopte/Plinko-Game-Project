      ...
      <main className="flex-1 w-full max-w-[1600px] mx-auto p-2 lg:p-4 overflow-hidden flex flex-col">
        {view === 'game' ? (
            <div className="flex flex-col lg:flex-row gap-4 h-full animate-fade-in overflow-y-auto lg:overflow-hidden pb-safe">
              ...
              {/* Game Board Section */}
              {/* Mobile: 50% viewport height, min 300px. Desktop: Fills remaining space. */}
              <div className="order-1 lg:order-2 shrink-0 w-full h-[50vh] min-h-[300px] lg:h-full lg:flex-1 bg-gradient-to-b from-slate-800/20 to-slate-900/80 rounded-3xl border border-slate-700/30 shadow-2xl relative overflow-hidden backdrop-blur-md flex flex-col p-1" role="application" aria-label="Plinko Board">
              ...
              {/* Controls Section */}
              {/* Mobile: Below board. Desktop: Left sidebar, scrollable. */}
              <div className="order-2 lg:order-1 w-full lg:w-[400px] flex flex-col gap-4 lg:h-full lg:overflow-y-auto custom-scrollbar pr-1 pb-8 lg:pb-0">
                ...
                <header className="shrink-0 hidden lg:block">
                  <p className="text-slate-400 text-xs flex items-center gap-2" role="status" aria-label="Game controls">
                    <span className="sr-only">Controls:</span>
                    <kbd className="bg-slate-800 px-1.5 py-0.5 rounded text-[10px] border border-slate-700 font-mono shadow-sm">←</kbd>
                    <kbd className="bg-slate-800 px-1.5 py-0.5 rounded text-[10px] border border-slate-700 font-mono shadow-sm">→</kbd> Aim
                    <span className="mx-1 text-slate-600">|</span>
                    <kbd className="bg-slate-800 px-1.5 py-0.5 rounded text-[10px] border border-slate-700 font-mono shadow-sm">Space</kbd> Drop
                  </p>
                </header>
                ...
                {lastResult && !isDropping && (
                  <div className="bg-slate-900/40 rounded-xl p-4 border border-slate-800 shadow-inner">
                    <h3 className="text-slate-500 text-[10px] font-bold uppercase tracking-wider mb-3">Fairness Details</h3>
                    <div className="flex flex-col gap-2 text-xs font-mono text-slate-300">
                      <p><span className="text-slate-500">Client:</span> {lastResult.clientSeed}</p>
                      <p><span className="text-slate-500">Nonce:</span> {lastResult.nonce}</p>
                      <div className="flex items-center gap-2">
                        <span className="text-slate-500">Server:</span>
                        {lastResult.serverSeed === "HIDDEN_UNTIL_REVEAL" ? (
                          <button
                            onClick={handleReveal}
                            className="bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/30 border border-cyan-500/50 px-2 py-0.5 rounded text-[10px] font-bold transition-all"
                          >
                            REVEAL SEED
                          </button>
                        ) : (
                          <span className="text-green-400 truncate max-w-[200px]" title={lastResult.serverSeed}>
                            {lastResult.serverSeed.substring(0, 16)}...
                          </span>
                        )}
                      </div>
                    </div>
                ...
                {/* History */}
                <div className="bg-slate-900/40 rounded-xl p-4 border border-slate-800 flex-1 min-h-[200px] flex flex-col shadow-inner overflow-hidden" aria-label="Game History">
                  <div className="flex justify-between items-end mb-3 shrink-0">
                     <h3 className="text-slate-500 text-[10px] font-bold uppercase tracking-wider">Recent Drops</h3>
                     <span className="text-[9px] text-slate-600">{history.length} games</span>
                  </div>
                  <div className="flex flex-col gap-2 overflow-y-auto pr-1 custom-scrollbar flex-1 max-h-[300px] lg:max-h-none">
                    {history.length === 0 && <div className="text-slate-600 text-xs italic text-center mt-6">No games played yet.</div>}
                    {history.slice().reverse().map((game, i) => (
                      <div key={i} className="flex flex-col gap-1 bg-slate-800/40 p-2 rounded-lg border border-slate-700/50 hover:bg-slate-800 transition-colors group shrink-0">
                        <div className="flex justify-between items-center">
                            <div className="flex items-center gap-2">
                               <div className={`w-1.5 h-1.5 rounded-full ${game.payout >= 1 ? 'bg-green-400 shadow-[0_0_8px_rgba(74,222,128,0.5)]' : 'bg-red-400'}`}></div>
                               <span className={`font-mono text-xs font-bold ${game.payout >= 1 ? 'text-green-400' : 'text-slate-400'}`}>
                                {game.payout}x
                               </span>
                            </div>
